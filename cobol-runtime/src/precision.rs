...
/// FUNCTION VARIANCE at arbitrary precision (f64 args).
pub fn precise_variance(values: &[f64], digits: usize) -> String {
    if digits <= 15 || values.is_empty() {
        if values.is_empty() { return format!("{:.*}", digits, 0.0); }
        let mean = values.iter().sum::<f64>() / values.len() as f64;
        let var = values.iter().map(|x| (x - mean) * (x - mean)).sum::<f64>() / values.len() as f64;
        return format!("{:.*}", digits, var);
    }
    let prec = wp(digits + 10);
    let n_f: DBig = format!("{}", values.len()).parse().unwrap();
    let n = sp(n_f, prec);
    ...
    let variance = sp(var_sum / &n, prec);
    format_dbig(&variance, digits)
}

/// FUNCTION VARIANCE with string inputs.
pub fn precise_variance_str(values: &[&str], digits: usize) -> String {
    if values.is_empty() { return format!("{:.*}", digits, 0.0); }
    if digits <= 15 {
        let vals: Vec<f64> = values.iter().map(|s| s.trim().parse().unwrap_or(0.0)).collect();
        return precise_variance(&vals, digits);
    }
    let prec = wp(digits + 10);
    let n_f: DBig = format!("{}", values.len()).parse().unwrap();
    let n = sp(n_f, prec);
...
/// FUNCTION PRESENT-VALUE with string inputs for precision.
pub fn precise_present_value_str(rate: &str, values: &[&str], digits: usize) -> String {
    if digits <= 15 || values.is_empty() {
        let r: f64 = rate.trim().parse().unwrap_or(0.0);
        let mut pv = 0.0f64;
        for (i, v) in values.iter().enumerate() {
            let vf: f64 = v.trim().parse().unwrap_or(0.0);
            pv += vf / (1.0 + r).powi((i + 1) as i32);
        }
        return format!("{:.*}", digits, pv);
    }
    let prec = wp(digits + 10);
    ...
    let mut pv = sp(DBig::ZERO, prec);
    for (i, v) in values.iter().enumerate() {
        let vb = str_to_dbig(v, digits + 10);
        let denom = pow_dbig(&base, (i + 1) as i64, prec);
        pv = sp(pv + sp(vb / denom, prec), prec);
    }
    format_dbig(&pv, digits)
