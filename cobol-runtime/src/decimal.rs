...
// pow for Decimal (COBOL COMPUTE x ** y)
impl Decimal {
    pub fn pow(self, exp: u32) -> Decimal {
        if exp == 0 { return Decimal::from_i64(1, self.scale); }
        let base = self.to_f64();
        let result = base.powi(exp as i32);
        let factor = 10f64.powi(self.scale as i32);
        Decimal { value: (result * factor).round() as i64, scale: self.scale }
    }
    pub fn to_i64(self) -> i64 { i64::from(self) }
    pub fn to_i32(self) -> i32 { i32::from(self) }
    pub fn to_u32(self) -> u32 { u32::from(self) }
    pub fn abs(self) -> Decimal { Decimal { value: self.value.abs(), scale: self.scale } }
}

// Div<u32> for Decimal
impl std::ops::Div<u32> for Decimal {
    type Output = Decimal;
    fn div(self, rhs: u32) -> Decimal { self / (rhs as i64) }
}
impl std::ops::DivAssign<u32> for Decimal {
    fn div_assign(&mut self, rhs: u32) { if rhs != 0 { self.value /= rhs as i64; } }
}

// f64 ↔ Decimal arithmetic (COBOL COMPUTE with mixed types)
impl std::ops::Mul<Decimal> for f64 {
    type Output = Decimal;
    fn mul(self, rhs: Decimal) -> Decimal {
        let result = self * rhs.to_f64();
        let factor = 10f64.powi(rhs.scale as i32);
        Decimal { value: (result * factor).round() as i64, scale: rhs.scale }
    }
}
impl std::ops::Mul<f64> for Decimal {
    type Output = Decimal;
    fn mul(self, rhs: f64) -> Decimal {
        let result = self.to_f64() * rhs;
        let factor = 10f64.powi(self.scale as i32);
        Decimal { value: (result * factor).round() as i64, scale: self.scale }
    }
}
impl std::ops::Div<f64> for Decimal {
    type Output = Decimal;
    fn div(self, rhs: f64) -> Decimal {
        if rhs == 0.0 { return Decimal::zero(self.scale); }
        let result = self.to_f64() / rhs;
        let factor = 10f64.powi(self.scale as i32);
        Decimal { value: (result * factor).round() as i64, scale: self.scale }
    }
}
impl std::ops::Div<Decimal> for f64 {
    type Output = Decimal;
    fn div(self, rhs: Decimal) -> Decimal {
        if rhs.value == 0 { return Decimal::zero(rhs.scale); }
        let result = self / rhs.to_f64();
        let factor = 10f64.powi(rhs.scale as i32);
        Decimal { value: (result * factor).round() as i64, scale: rhs.scale }
    }
}
impl std::ops::Add<Decimal> for f64 {
    type Output = Decimal;
    fn add(self, rhs: Decimal) -> Decimal {
        let result = self + rhs.to_f64();
        let factor = 10f64.powi(rhs.scale as i32);
...
impl std::ops::Sub<Decimal> for f64 {
    type Output = Decimal;
    fn sub(self, rhs: Decimal) -> Decimal {
        let result = self - rhs.to_f64();
        let factor = 10f64.powi(rhs.scale as i32);
...
impl std::ops::Add<f64> for Decimal {
    type Output = Decimal;
    fn add(self, rhs: f64) -> Decimal {
        let result = self.to_f64() + rhs;
        let factor = 10f64.powi(self.scale as i32);
        Decimal { value: (result * factor).round() as i64, scale: self.scale }
    }
}
impl std::ops::Sub<f64> for Decimal {
    type Output = Decimal;
    fn sub(self, rhs: f64) -> Decimal {
        let result = self.to_f64() - rhs;
        let factor = 10f64.powi(self.scale as i32);
        Decimal { value: (result * factor).round() as i64, scale: self.scale }
    }
}
impl std::ops::MulAssign<f64> for Decimal {
    fn mul_assign(&mut self, rhs: f64) {
        let result = self.to_f64() * rhs;
        let factor = 10f64.powi(self.scale as i32);
        self.value = (result * factor).round() as i64;
    }
}
impl std::ops::DivAssign<f64> for Decimal {
...
// Decimal ↔ u32 division (u32 / Decimal)
impl std::ops::Div<Decimal> for u32 {
    type Output = u32;
    fn div(self, rhs: Decimal) -> u32 { let v = i64::from(rhs); if v != 0 { (self as i64 / v) as u32 } else { 0 } }
}

// u32 ↔ i32 comparisons (COBOL mixes signed/unsigned)
// Cannot impl PartialEq/PartialOrd for foreign types (orphan rule),
// but we can provide helpers via Decimal comparisons

// Decimal cast support (for transpiled `d as i64` patterns)
impl From<Decimal> for usize {
    fn from(d: Decimal) -> usize { i64::from(d) as usize }
}
